...
safe_down() {
	echo "Tearing down GitOps-managed workloads (safe mode)..."

	# Suspend reconciliation so workloads don't get recreated while we're deleting namespaces
	if kubectl -n flux-system get kustomization flux-system >/dev/null 2>&1; then
		echo "Suspending Flux reconciliation (kustomization/flux-system)..."
		kubectl -n flux-system patch kustomization flux-system --type merge -p '{"spec":{"suspend":true}}' >/dev/null
	else
		echo "Note: Flux kustomization not found; skipping suspend."
	fi

	# Delete non-system namespaces but keep core/system and Flux
	echo "Deleting non-system namespaces (keeping kube-* / default / flux-system / local-path-storage)..."
	for ns in $(kubectl get ns -o jsonpath='{.items[*].metadata.name}'); do
		case "$ns" in
			kube-system|kube-public|kube-node-lease|default|flux-system|local-path-storage)
				continue
				;;
			*)
	...
	echo "✓ Safe teardown complete."
	echo "To resume reconciliation:"
	echo "  kubectl -n flux-system patch kustomization flux-system --type merge -p '{\"spec\":{\"suspend\":false}}'"
...
destroy_all() {
	...
	echo "Tearing down GitOps stack (destroy mode)..."
