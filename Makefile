...
status:
	@echo "=== Flux System Status ===" && \
	 flux get kustomization -n flux-system || true && \
	 echo "" && \
	 echo "=== Detailed Kustomization Status ===" && \
	 for ks in flux-system home-secrets home-infra home-infra-config home-apps; do \
	   echo "--- $$ks ---"; \
	   kubectl get kustomization $$ks -n flux-system -o jsonpath='{.status.conditions[?(@.type=="Ready")].message}' 2>/dev/null || echo "Not found"; \
	 done && \
	 echo "" && \
	 echo "=== Core Namespaces ===" && \
	 kubectl get ns cert-manager ingress-nginx metallb-system apps 2>/dev/null || echo "Some namespaces not ready" && \
	 echo "" && \
	 echo "=== HelmReleases ===" && \
	 flux get helmrelease -n ingress-nginx 2>/dev/null || echo "ingress-nginx: not deployed" && \
	 flux get helmrelease -n metallb-system 2>/dev/null || echo "metallb: not deployed" && \
	 flux get helmrelease -n cert-manager 2>/dev/null || echo "cert-manager: not deployed" && \
	 echo "" && \
	 echo "=== Pods by Namespace ===" && \
	 echo "cert-manager:" && kubectl get pods -n cert-manager --no-headers 2>/dev/null | head -5 || echo "  (not deployed)" && \
	 echo "ingress-nginx:" && kubectl get pods -n ingress-nginx --no-headers 2>/dev/null | head -5 || echo "  (not deployed)" && \
	 echo "metallb-system:" && kubectl get pods -n metallb-system --no-headers 2>/dev/null | head -5 || echo "  (not deployed)" && \
	 echo "apps:" && kubectl get pods -n apps --no-headers 2>/dev/null | head -5 || echo "  (not deployed)"

verify-structure:
	@echo "=== Verifying Kustomize Structure ===" && \
	 echo "Checking: k8s/infrastructure/base" && \
	 kustomize build k8s/infrastructure/base >/dev/null && echo "  ✓ Valid" || echo "  ✗ FAILED" && \
	 echo "Checking: k8s/infrastructure/controllers" && \
	 kustomize build k8s/infrastructure/controllers >/dev/null && echo "  ✓ Valid" || echo "  ✗ FAILED" && \
	 echo "Checking: k8s/infrastructure/config" && \
	 kustomize build k8s/infrastructure/config >/dev/null && echo "  ✓ Valid" || echo "  ✗ FAILED" && \
	 echo "Checking: k8s/infrastructure/secrets" && \
	 kustomize build k8s/infrastructure/secrets >/dev/null && echo "  ✓ Valid" || echo "  ✗ FAILED" && \
	 echo "Checking: k8s/apps/home" && \
	 kustomize build k8s/apps/home >/dev/null && echo "  ✓ Valid" || echo "  ✗ FAILED" && \
	 echo "Checking: k8s/clusters/home (root)" && \
	 kustomize build k8s/clusters/home >/dev/null && echo "  ✓ Valid" || echo "  ✗ FAILED"
